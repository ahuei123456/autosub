...
def format(
    ctx: typer.Context,
    input_transcripts: list[Path] = typer.Argument(
        ...,
        help="One or more input transcript.json files generated by the transcribe command.",
        exists=True,
        dir_okay=False,
    ),
    ...
    json_out: Path = typer.Option(
        None,
        "--json-out",
        help="Path to save the formatted JSON passed to the translate stage.",
    ),
    ...
    out = resolved["out"]
    json_out = resolved["json_out"]
    keyframes = resolved["keyframes"]
    fps = resolved["fps"]
    profile = resolved["profile"]

    if not out:
        out = input_transcripts[0].with_name("original.ass")
    if not json_out:
        json_out = out.with_name("formatted.json")
...
def translate(
    ...
    input_json: Path = typer.Argument(
        ...,
        help="Path to the formatted JSON generated by the format command.",
        exists=True,
        dir_okay=False,
    ),
    out: Path = typer.Option(
        None,
        "--out",
        help="Path to save the translated JSON (defaults to translated.json in the same directory).",
    ),
    ass_out: Path = typer.Option(
        None,
        "--ass-out",
        help="Path to save the rendered translated .ass byproduct.",
    ),
    ...
    """
    Step 3: Translates formatted JSON using the configured Translation Engine.
    """
    resolved = apply_command_config(
        ctx,
        "translate",
    ...
    if not out:
        out = input_json.with_name("translated.json")
    if not ass_out:
        ass_out = out.with_suffix(".ass")
    ...
    if save_log:
        ...
        log_stem = input_json.stem.removesuffix("_formatted")
        translate_log_dir = input_json.parent / f"{log_stem}_logs_{ts}"
...
def postprocess(
    ...
    input_json: Path = typer.Argument(
        ...,
        help="Path to the translated JSON generated by the translate command.",
        exists=True,
        dir_okay=False,
    ),
    out: Path = typer.Option(
        None,
        "--out",
        help="Path to save the postprocessed JSON (defaults to postprocessed.json in the same directory).",
    ),
    ass_out: Path = typer.Option(
        None,
        "--ass-out",
        help="Path to save the rendered final .ass byproduct.",
    ),
    profile: str = typer.Option(
        None,
        "--profile",
...
def run(
    ...
    formatted_json_out = out_dir / f"{stem}_formatted.json"
    original_ass_out = out_dir / f"{stem}_original.ass"
    translated_json_out = out_dir / f"{stem}_translated.json"
    translated_ass_out = out_dir / f"{stem}_translated.ass"
    postprocessed_json_out = out_dir / f"{stem}_postprocessed.json"
    final_ass_out = out_dir / f"{stem}_final.ass"
    ...
    try:
        ...
        format_module.format_subtitles(
            transcript_out,
            original_ass_out,
            output_json_path=formatted_json_out,
            keyframes=kf_ms,
    ...
    try:
        ...
        translate_module.translate_subtitles(
            formatted_json_out,
            translated_json_out,
            output_ass_path=translated_ass_out,
            system_prompt=final_prompt,
            target_lang=target_lang,
            source_lang=source_lang,
            bilingual=bilingual,
    ...
    try:
        ...
        postprocess_module.postprocess_subtitles(
            translated_json_out,
            output_json_path=postprocessed_json_out,
    ...
    logger.info(
        f"Pipeline completed successfully! Final output saved to {final_ass_out}"
    )
